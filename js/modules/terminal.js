...
import { TERMINAL_CONFIG, COMMANDS, RESUME_FLAGS, VIRTUAL_FILES } from './constants.js';
/**
 * @module Terminal
...
export function initializeTerminal() {
    const { terminalCommandForm, terminalInput, terminalHistory } = dom;
    if (!terminalCommandForm || !terminalInput || !terminalHistory)
        return;
    ...
    terminalCommandForm.addEventListener('submit', (e) => {
        e.preventDefault();
    ...
    function processCommand(rawCommand) {
        ...
        else if (cmd !== COMMANDS.RESUME && !Object.values(COMMANDS).includes(cmd)) {
        ...
        if (cmd === COMMANDS.RESUME) {
            ...
            if (flag === '--right' || flag === '-r') {
                if (subflag === 'on') {
                    window.toggleSecurityProtection?.(true);
                    appendLine('Security protection enabled.', 'output-text');
                }
                else if (subflag === 'off') {
                    window.toggleSecurityProtection?.(false);
