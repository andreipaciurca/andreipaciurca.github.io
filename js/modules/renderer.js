/**
 * @module Renderer
 * @description Builds HTML markup strings for the resume's interactive UI sections.
 * Each function is pure: takes data + a translate function, returns an HTML string.
 */
  ...
  return experiences
    .map(function mapExperience(exp) {
      ...
      return (
        ...
        escapeHtml(translate(exp.location)) + "</span>" +
        "</div>" +
        '<h3 class="experience-title">' +
        escapeHtml(translate(exp.title)) + " · " + escapeHtml(exp.company) +
        "</h3>" +
        "</div>" +
  ...
  return skillGroups
    .map(function mapGroup(group, index) {
      ...
      return (
        ...
        '<h3 class="skill-group-title ' + escapeHtml(groupClass) + '">' +
        escapeHtml(translate(group.label)) + "</h3>" +
