...
import { escapeHtml, truncateAtWordBoundary, countWords, extractPublicPath, toSafeExternalUrl, isLikelyEmailAddress, isLikelyPhoneNumber, } from './utils.js';
...
export function optimizePrintResumeLayout() {
    ...
    for (const step of printConfig.strategySteps) {
        const payload = renderPrintResume({
            maxBulletsPerExperience: Math.min(printResumeSettings.maxBulletsPerExperience ?? 3, step.maxBulletsPerExperience),
